...
interface EditTradeRowProps {
  ...
  onSave: (data: CreateTradeRequest) => void;
  onCancel: () => void;
}

function EditTradeRow({ defaultValues, onSave, onCancel }: EditTradeRowProps) {
  const { register, handleSubmit, formState: { errors } } = useForm<CreateTradeRequest>({ defaultValues });

  const submit = (data: CreateTradeRequest) => {
    onSave({
      ...data,
  ...
  return (
    <tr className="bg-blue-50">
      ...
      <td className="px-4 py-2 text-right whitespace-nowrap">
        <button type="button" onClick={handleSubmit(submit)} className="text-green-600 hover:text-green-800 text-xs mr-2">Save</button>
...
export default function TradesPage() {
  ...
  const onAdd = (data: CreateTradeRequest) => {
    dispatch(addTradeRequest({ portfolioId, holdingId, data }));
    setAddingNew(false);
  };

  const onUpdate = (tradeId: number, data: CreateTradeRequest) => {
    dispatch(updateTradeRequest({ portfolioId, holdingId, tradeId, data }));
    setEditingId(null);
  };
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y">
              {addingNew && (
                <EditTradeRow
                  key="new"
                  defaultValues={{ date: new Date().toISOString().split('T')[0], type: 'BUY', quantity: 0, price: 0, fee: undefined }}
              ...
              {trades.map((t) => {
                ...
                if (editingId === t.id) {
                  return (
                    <EditTradeRow
                      key={t.id}
                      defaultValues={{ date: t.date, type: t.type, quantity: t.quantity, price: t.price, fee: t.fee ?? undefined }}
    
                      onSave={(data) => onUpdate(t.id, data)}
                      onCancel={cancelEdit}
                    />
                  );
                }
                ...
                return (
                  <tr key={t.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-right whitespace-nowrap">
                      <button
                        type="button"
                        onClick={() => { setAddingNew(false); setEditingId(t.id); }}
